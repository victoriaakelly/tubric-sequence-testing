...
def infotodict(seqinfo):
    ...
    info = {t1w: [], t2w: [], tapping: [], reward: [], mag: [], phase: []}
    ...
    for s in seqinfo:
        ...
        if (s.dim3 == 192) and (s.dim4 == 1) and ('t2' in s.protocol_name):
            info[t2w] = [s.series_id]
        if (s.dim4 == 140) and ('ep2d_bold' in s.protocol_name):
            info[tapping].append({'item': s.series_id})
        if ((s.dim4 == 190) or (s.dim4 == 306)) and ('ep2d_bold' in s.protocol_name):
            info[reward].append({'item': s.series_id})
        if (s.dim4 == 228) and ('17-ep2d' in s.series_id):
            info[tapping].append({'item': s.series_id})
        if (s.dim4 == 228) and ('19-ep2d' in s.series_id):
            info[reward].append({'item': s.series_id})
