...
def infotodict(seqinfo):
    ...
    for s in seqinfo:
        ...
        #functionals. Not sure why the last FT is long, but I think we copied the RT one
        if (135 < s.dim4 < 235) and ('ep2d_FT' in s.protocol_name):
            info[tapping].append({'item': s.series_id})
        if ((s.dim4 == 290) or (s.dim4 == 468)) and ('ep2d_RT' in s.protocol_name):
            info[reward].append({'item': s.series_id})
        if (s.dim4 == 468) and ('22-ep2d_RT-sms-2mm-fast' in s.series_id):
            info[reward].append({'item': s.series_id})
        if (s.dim4 == 468) and ('21-ep2d_RT-sms-2mm-fast' in s.series_id):
            info[tapping].append({'item': s.series_id})

        #resting param tests. Check against notes from Victoria/Liz?
        #Did pre-scan norm (on/off) and gap (on/off). did one of the four twice by mistake
        if (s.dim4 == 100) and ('ep2d' in s.protocol_name):
